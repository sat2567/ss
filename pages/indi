...
def fetch_data(ticker_dict, period="2y"):
    # Fetch a minimum of 1 month of data so technical indicators (like RSI and MA) don't break
    fetch_period = "1mo" if period in ["1wk", "2wk"] else period
    data_dict = {}
    ticker_list = list(ticker_dict.values())
    try:
        raw = yf.download(ticker_list, period=fetch_period, group_by='ticker', auto_adjust=True, progress=False)
    except Exception:
        return {}
    ...
    for name, ticker in ticker_dict.items():
        try:
            ...
            # Slice the data to strictly reflect the 1 or 2 week period for visualization
            if period in ["1wk", "2wk"]:
                days_to_keep = 7 if period == "1wk" else 14
                cutoff_date = df.index.max() - pd.Timedelta(days=days_to_keep)
...
def resolve_smallcap(period, uploaded_file=None):
    ...
    """
    period_days = {"1wk": 7, "2wk": 14, "1mo": 30, "3mo": 91, "6mo": 182,
...
def signal_summary_html(df):
    ...
    # Guard against not having enough data points for 50/200 MA cross logic
    if len(close) >= 200:
...
def stats_panel_html(df, period_label):
    ...
    # Need 252 days for 52W High/Low. Use min/max of current data if < 252 points.
    lookback = min(252, len(close))
    high52  = close.rolling(lookback).max().iloc[-1]
    low52   = close.rolling(lookback).min().iloc[-1]

    current = close.iloc[-1]
    from_high = ((current - high52) / high52) * 100 if high52 else 0
    from_low  = ((current - low52)  / low52)  * 100 if low52 else 0
...
def build_vol_chart(index_data, selected):
    ...
    for i, name in enumerate(selected):
        if name in index_data and not index_data[name].empty:
            ...
            # If timeframe is too small for 21-day rolling, avoid plotting
            if len(rets) >= 21:
...
def main():
    ...
    with st.sidebar:
        ...
        selected_period_label = st.selectbox("LOOKBACK PERIOD", list(period_options.keys()), index=5)
    ...
    with tab5:
        ...
        with c_dist:
            ...
            # Avoid rolling operations if there isn't enough data
            if len(rets) >= 63:
