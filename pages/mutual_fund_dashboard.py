...
# -----------------------------------------------------------------
# --- Auto refresh logic (refresh once every day after 9 AM) -------
# -----------------------------------------------------------------
def should_refresh():
    ...
    if now >= today_9am:
        if "last_refresh_date" not in st.session_state or st.session_state["last_refresh_date"] != now.date():
            ...
            st.session_state["last_extracted"] = now   # <-- store last extracted timestamp
            st.cache_data.clear()
            return True
    return False
...
# -----------------------------------------------------------------
# --- Fetch a table from Moneycontrol --------------------------------
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# --- Scrape returns + rank table and merge ------------------------
# -----------------------------------------------------------------
def scrape_category(category, category_label):
    ...
    # convert numeric return / NAV columns
    for col in combined.columns:
        if any(period in col for period in
               ['1W', '1M', '3M', '6M', '1Y', '2Y', '3Y', '5Y', '10Y', 'YTD', 'Return', 'Change']):
...
def main():
    ...
    if st.session_state.get('needs_refresh', False):
        ...
        st.rerun()  # replaced old experimental rerun
    ...
    # If first time, set extracted time
    if "last_extracted" not in st.session_state:
        st.session_state["last_extracted"] = datetime.datetime.now()

    last_time = st.session_state["last_extracted"]
    last_time_str = last_time.strftime("%d-%m-%Y %I:%M %p")

    st.markdown(f"### 🟠 Last Extracted: **{last_time_str}**")
