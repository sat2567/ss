    ...
    if not df_ranks.empty:
        if "Scheme Name" in df_returns.columns and "Scheme Name" in df_ranks.columns:
            ...
            if "Plan" in df_returns.columns and "Plan" in df_ranks.columns:
                keys.append("Plan")
            df_returns = df_returns.merge(df_ranks.drop(columns=["Category Name"], errors="ignore"), on=keys, how="left")

    # Filter regular plans if Plan column exists
    if "Plan" in df_returns.columns:
        df_returns = df_returns[df_returns["Plan"].str.lower().str.contains("regular", na=False)]

    # Prefer Growth options if Option column exists
    if "Option" in df_returns.columns and df_returns["Option"].str.lower().str.contains("growth", na=False).any():
        df_returns = df_returns[df_returns["Option"].str.lower().str.contains("growth", na=False)]
    # Else try scheme name contains growth
    elif "Scheme Name" in df_returns.columns and df_returns["Scheme Name"].str.lower().str.contains("growth", na=False).any():
        df_returns = df_returns[df_returns["Scheme Name"].str.lower().str.contains("growth", na=False)]
...
def aggregate_categories(selected_categories):
    dataframes = []
    with ThreadPoolExecutor(max_workers=len(selected_categories)) as executor:
        futures = {executor.submit(scrape_category, CATEGORIES[cat]): cat for cat in selected_categories}
...
def main():
    ...
    if category == "All Funds":
        ...
        with st.spinner("Fetching data for selected categories..."):
            data = aggregate_categories(selected_cats)
    else:
        with st.spinner(f"Fetching {category} funds data..."):
            data = scrape_category(CATEGORIES[category])
    ...
    if not data.empty:
        ...
        filtered_data = data
        if search_text:
            filtered_data = filtered_data[filtered_data["Scheme Name"].str.contains(search_text, case=False, na=False)]
